...
func _ready() -> void:
	...
	arrow_2.global_position.y=spawn_location.global_position.y-50
	arrow_3.global_position=spawn_location.global_position
	arrow_3.global_position.x=spawn_location.global_position.x+50
	arrow_4.global_position=spawn_location.global_position
	arrow_4.global_position.y=spawn_location.global_position.y+50
	...
	if targets.get_children().size()!=0:
		...
		specTarg=targets.get_children()[randi_range(0,targets.get_children().size()-1)]
		var tween2 = create_tween()
		tween2.tween_property(arrow_2,"global_position",specTarg.global_position,1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
		await get_tree().create_timer(0.43).timeout
		specTarg=targets.get_children()[randi_range(0,targets.get_children().size()-1)]
		var tween3 = create_tween()
		tween3.tween_property(arrow_3,"global_position",specTarg.global_position,1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
		await get_tree().create_timer(0.43).timeout
		specTarg=targets.get_children()[randi_range(0,targets.get_children().size()-1)]
		var tween4 = create_tween()
		tween4.tween_property(arrow_4,"global_position",specTarg.global_position,1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
