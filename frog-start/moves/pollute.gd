...
func _ready() -> void:
	if bone==1:
		strike_projectile.defRev+=5
		sl_1.modulate=Color(1.248, 0.0, 1.399, 1.0)
	if bone==2:
		strike_projectile.strRev+=5
		sl_1.modulate=Color(1.516, 0.652, 0.0, 1.0)
		strike_projectile.spdRev+=5
...
func _on_targe_area_entered(area: Area2D) -> void:
	...
	await get_tree().create_timer(0.1).timeout
	sl_1.global_position.x=targe.global_position.x
	sl_1.global_position.y=targe.global_position.y-57
	sl_1.visible=true
	sl_1.play("default")
